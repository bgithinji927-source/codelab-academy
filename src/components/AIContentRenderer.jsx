...
export function CodeBlock({ block }) {
  ...
  return <section className="kai-rich-code" aria-label={`${language} code example`}>
    ...
    <Editor height={Math.min(420, Math.max(150, value.split("\n").length * 21 + 24))} language={language === "py" ? "python" : language} value={value} onChange={(next) => setValue(next || "")} theme="vs-dark" options={{ minimap: { enabled: false }, lineNumbers: "on", lineNumbersMinChars: 4, lineDecorationsWidth: 12, glyphMargin: false, fontSize: 14, padding: { top: 12, bottom: 12, left: 12, right: 8 }, scrollBeyondLastLine: false, scrollBeyondLastColumn: 0 }} />
